...
their own unverified list in §13-U(v2.1). One structural caveat applied to the 2026-08-24
...
**v2.1, second pass (2026-09-16, after the merged-main adversarial sweep of `7b88c2e`). No decision,
table or WP boundary changed; twelve findings folded in, each dated.** The sweep reproduced every
...
(6) Small corrections: §3.3's "three of four cases", §1.2's stale notice text, §6.5's reader count,
...
`.gitignore` paragraph, since 2026-09-12 — row 22 stays UNOWNED), so an adopted project's out-of-band-commit
...
| 20 | `.claude/settings.local.json` — `mcpServers.qdrant` (`# BL-233-WPB-MANIFEST-DECL`) | **UNOWNED** — `## BL-233:`'s accumulation gate derives NOT REQUIRED and switches off silently |
...
**Sequencing: `## BL-225:` first, then WP9 → WP10 → WP11 → WP12a → WP12b → WP7.** BL-225 (the driver
...
3. **The init-parity audit is DELIVERED at §8.7a, and it is worse news than this entry expected.**
   **34 rows** (33 until `## BL-284:`'s finding added row 32 on 2026-09-16; 32 until WP9b added row 22a); the adoption half measured by execution — re-measured 2026-09-16, §13-V27 — the `init.sh` half read from source
   ...
   asked for and all it asked for.
...
10. **That `templates/tool-matrix/` keeps `gitleaks` `"required": true`.** §6.2's "D2 asks nothing
    ...
    *(Re-verified 2026-09-16: `"required": true` at the `gitleaks` entry, §13-V19; the WP10a suite
    ...
    assert the floor is ≥ 8.19.0 and that the resolver adoption calls enforces `min_version`, or 10b
    must probe the subcommand itself; at `7b88c2e` the floor is 8.18.0 and nothing on adoption's
    path reads it.)*
...
- **Second pass (2026-09-16, after the sweep):** every correction above is dated to `01b66e3` or
  ...
  hosts, and the WP10a split's attribution to Karl.
